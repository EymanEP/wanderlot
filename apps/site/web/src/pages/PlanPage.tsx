...
import { longDate, shortDate, type Category } from "@wanderlot/core";
...
export function PlanPage() {
  ...
  const { plan, destinations, myRanking, ballots, comments, members, now, result, closed } = site;
  ...
  const winner = result?.winnerId ? destinations.find((d) => d.id === result.winnerId) : undefined;
  ...
  return (
    <Main>
      <PageHeader
        size="display"
        title={plan.name}
        subtitle={`Del ${weekday(from)} ${from.split(" ")[1]} al ${weekday(to)} ${to.split(" ")[1]} · salida desde Madrid · ${numberWord(destinations.length)} propuestas sobre la mesa`}
        actions={
          <>
            <div className="flex flex-col gap-0.5 lg:items-end">
              {closed ? (
                <>
                  ...
                  <span className="text-[13px] text-muted">Votasteis los {ballots.length}</span>
                </>
              ) : (
                <>
                  <span className="text-[15px] font-bold">
                    {ballots.length} de {plan.partySize} habéis votado
                  </span>
        ...
        <div className="flex flex-wrap items-center gap-3.5">
          ...
          {site.otherPlans.map((p) => (
            <Link key={p.id} to={`/p/${p.id}`} className={chipClasses("nav", p.status === "draft")}>
              {p.name} · {p.status === "draft" ? "borrador" : p.status === "closed" ? placeName(p.winnerDestinationId ?? "") : "votando"}
            </Link>
          ))}
        </div>
        {checkedAt && (
