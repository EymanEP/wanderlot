...
export function Preview() {
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-2 bg-ink px-4 py-2 text-[13px] text-white">
        ...
        <div role="group" aria-label="Estado de la votación" className="flex gap-1 rounded-full bg-white/10 p-1">
          <Chip size="sm" variant="subtle" on={!closed} onClick={() => setClosed(false)} className={closed ? "text-white/80 hover:bg-white/10" : ""}>
            Votación abierta
          </Chip>
          <Chip size="sm" variant="subtle" on={closed} onClick={() => setClosed(true)} className={!closed ? "text-white/80 hover:bg-white/10" : ""}>
            Votación cerrada
          </Chip>
        </div>
      </div>
